...
def run_6_body_simulation( sep, ecc, direction, v_coms,orbit_plane, impact_parameter , distance, masses = None, centers = None,age = None,run_label=""):
    """
    ...
    if centers == None:
        centers = [ [0,0,0], [ distance[0],distance[0],distance[0]], [distance[1],distance[1],distance[1]]]  # in AU
    ...
    grav_particles = make_triple_binary_system(masses, sep, ecc, direction, orbit_plane,impact_parameter, centers, v_coms)
    ...
    while t < t_end:
        ...
        if pair:
            ...
            success, remnant = collision(i, j, n_collision, gravity, seba, t, key_map, run_label)
            ...
            if success:
                ...
                if remnant is None:
                    print(f"Destructive collision: terminating simulation at t = {t.value_in(units.yr):.1f} yr.")
...
def collision(i, j, n_collision, gravity, seba, t, key_map, run_label=""):
    ...
    try:
        ...
        progenitor_com_pos = (p_i.mass * p_i.position + p_j.mass * p_j.position) / (p_i.mass + p_j.mass)
        progenitor_com_vel = (p_i.mass * p_i.velocity + p_j.mass * p_j.velocity) / (p_i.mass + p_j.mass)
