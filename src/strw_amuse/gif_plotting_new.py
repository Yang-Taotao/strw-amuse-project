...
def visualize_frames(frames, run_label="test"):
    ...
    final_pos = np.array([final_particle.x.value_in(units.AU), final_particle.y.value_in(units.AU)])
    ...
    for f in frames:
        ...
        if max_index < len(f):
            p = f[max_index]
            tracked_positions_list.append([p.x.value_in(units.AU), p.y.value_in(units.AU)])
        else:
...
def visualize_initial_final_frames(frames, init_params, collision, label=""):
    ...
    if hasattr(initial[0], "vx"):
        ...
        ax1.quiver(xi_rel, yi_rel, vx, vy, angles="xy", scale_units="xy",
                   scale=1.5, color="gray", alpha=0.6, width=0.004)
    ...
    if hasattr(final[0], "vx"):
        ...
        ax2.quiver(x2_rel, y2_rel, vx*40, vy*40,
