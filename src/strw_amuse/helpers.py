...
def make_binary(
    ...
    pos1_base = np.array([r1.value_in(units.AU)*np.cos(f), r1.value_in(units.AU)*np.sin(f), 0.0])
    pos2_base = np.array([r2.value_in(units.AU)*np.cos(f), r2.value_in(units.AU)*np.sin(f), 0.0])
    vel1_base = np.array([-v1.value_in(units.kms)*np.sin(f), v1.value_in(units.kms)*np.cos(f), 0.0])
    vel2_base = np.array([-v2.value_in(units.kms)*np.sin(f), v2.value_in(units.kms)*np.cos(f), 0.0])
...
def outcomes(initial_particles, final_particles, collision_history,
