...
def make_duo_trinary_system( masses,
    seps,
    ecc,
    directions,
    orbit_plane,
    ...
    psi=None):
    "This function does the same as for the binary case, but now for triplets"
     # Default centers
    if centers is None:
        centers = [[-300, 0, 0], [300, 0, 0]]
    if v_coms is None:
        v_coms = [[0, 0, 0], [10, 0, 0]]  # first binary at rest
    if phases is None:
        phases = [0.0, 0.0]
    if psi is None:
        psi = [0.0]
   
    ma1, ma2, ma3, mb1, mb2, mb3 = masses
    sep_a1,sep_a2, sep_b1, sep_b2 = seps
    dir_a, dir_b = directions
    ecc_a1,ecc_a2, ecc_b1, ecc_b2 = ecc
    f_a1, f_a2, f_b1,f_b2 = phases
    psi_b = psi[0]
    ...
    f_a1,
    f_a2,  # true anomaly
    0.0,  # impact orientation
    )
    p4, p5,p6 = make_triplet(
    mb1,
    mb2,
    mb3,
    sep_b1| units.AU,
    ...
    dir_b,
    orbit_plane[0],
    impact_parameter[0] | units.AU,
    f_b1,
    f_b2,  # true anomaly
    psi_b,  # impact orientation
    )
    

    # Name particles
    p1.name, p2.name,p3.name = "A1", "A2","A3"
    p4.name, p5.name,p6.name = "B1", "B2","B3"
...
def critical_velocity_bin(masses, sep, ecc):
    """
...
def critical_velocity_trip(masses, sep, ecc):
    ...
    energy_a = (-G * m1 * m2 / (2 * a_1 * (1 - e_a1**2)))+(-G * (m1+m2)*m3 / (2 * a_2 * (1 - e_a2**2)))
    energy_b = (-G * m4 * m5 / (2 * b_1 * (1 - e_b1**2)))+(-G * (m4+m5)*m6 / (2 * b_2 * (1 - e_b2**2)))
...
def transformation_to_cartesian_bin(
    sep,
    true_anomalies,
    ecc,
    theta,
    ...
    # --------------------------
    v_crit = critical_velocity_bin([50.0] * 6, sep, ecc)
